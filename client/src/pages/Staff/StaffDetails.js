...
import {
    ...
    FiCalendar, FiClock, FiAward, TbCurrencyRupee,
...
const StaffDetails = () => {
    ...
    const handleDelete = async () => {
        if (window.confirm(`Are you sure you want to permanently delete ${staff?.user?.name}? This action cannot be undone.`)) {
            try {
    ...
    const departmentColors = {
        ...
        teacher: 'bg-primary-100 text-primary-800',
                ...
                <div className="mb-8">
                    ...
                    <div className="bg-white rounded-xl shadow-md p-6 mb-6">
                        <div className="flex flex-col md:flex-row items-start md:items-center">
                            {/* Avatar */}
                            <div className="h-24 w-24 bg-gradient-to-r from-primary-500 to-primary-600 rounded-full flex items-center justify-center text-white text-3xl font-bold mr-6">
                    ...
                    <div className="border-b border-gray-200 mb-6">
                        <nav className="flex space-x-8">
                            ...
                            <button
                                ...
                                className={`py-4 px-1 border-b-2 font-medium text-sm transition ${activeTab === 'children'
                                    ? 'border-primary-600 text-primary-600'
                                    : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`py-4 px-1 border-b-2 font-medium text-sm transition ${activeTab === 'reports'
                                    ? 'border-primary-600 text-primary-600'
                                    : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className={`py-4 px-1 border-b-2 font-medium text-sm transition ${activeTab === 'documents'
                                    ? 'border-primary-600 text-primary-600'
                                    : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                                    }`}
                            >
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'].map(day => (
                                            <span
                                                key={day}
                                                className={`px-3 py-1.5 rounded-lg text-sm font-medium ${staff.workingDays?.includes(day)
                                ...
                                {staff.user?.assignedChildren?.length > 0 ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {staff.user.assignedChildren.map(child => (
                                            ...
                                            >
                                                <div className="h-12 w-12 bg-gradient-to-r from-primary-100 to-primary-200 rounded-full flex items-center justify-center mr-4">
                                                    {child.photo ? (
                                                        <img
                                                            src={child.photo}
                                                            alt={child.name}
                                                ...
                                                <span className={`px-2 py-1 rounded-full text-xs ${child.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                                                    }`}>
                                                    {child.status}
                                                </span>
                                            </Link>
